...
public class fight_handler
{
    ...
    public fight_handler(String path2fight, content_handler con, boolean on)
    {
        ...
        if (!online) {
            ...
            non_walkable_image = "/data/pics/non_walkable.png";
                ...
                for (Object ob : l) {
                    ...
                    try {
                        if (p.cur != null && p.cur.visible) {
                            ...
                            if (p.cur.did_fight) {
                                g.drawImage(content.iml.get_img("/data/pics/Icons/Geschlagen.png"), x, y, place_width, place_height, imo);
                            }
                            if (p.cur.did_walk) {
                                g.drawImage(content.iml.get_img("/data/pics/Icons/Gegangen.png"), x, y, place_width, place_height, imo);
                            }
                            ...
                            if (p.cur == content.ini_table.get(0)) {
                                g.drawImage(content.iml.get_img("/data/pics/Icons/ist_dran.png"), x, y, place_width, place_height, imo);
                            }
                        }
                        else if (p.special.equals("NON-WALKABLE")) {
                            g.drawImage(content.iml.get_img(non_walkable_image), x, y, place_width, place_height, imo);
                        }
                    }
        ...
        try {
            ...
            if (show_move_radius) {
                ...
                for (Object o : in_reach) {
                    ...
                    int x = calc_offset(p.index).get(0);
                    int y = calc_offset(p.index).get(1);
                    g.drawImage(content.iml.get_img("/data/pics/Icons/attackierbar.png"), x, y, place_width, place_height, imo);
                }
            }
        }
    ...
    public void move_char(Place from, Place to, boolean human) {
        ...
        if (!was_fighting) {
            ...
            if (human) {
                ...
                //content.sol.get_sound("/data/sounds/moving.wav").loop_sound();
    ...
    public boolean attack_char(Place attacker, Place defender) {
        // very simple system
        float high = 1 + (float)attacker.cur.property_current.get("attackenmodifikator")/(float)10;
        float low = 1 - (float)attacker.cur.property_current.get("attackenmodifikator")/(float)10;
        if (low < 0) low = 0;
        ...
        high = 1 + (float)attacker.cur.property_current.get("verteidigungmodifikator")/(float)10;
        low = 1 - (float)attacker.cur.property_current.get("verteidigungmodifikator")/(float)10;
        if (low < 0) low = 0;
        float def_mod = ((float)Math.random() * (high - low) + low);

        int def = (int)Math.round(defender.cur.property_current.get("verteidigungspunkte") * def_mod);
    ...
    public void animate_way(Place start, Place aim, int i, int curi) {
        ...
        if (i == way.get(curi).size()-1) {
            ...
            //content.sol.get_sound("/data/sounds/moving.wav").stop_sound();
            return;
        }
